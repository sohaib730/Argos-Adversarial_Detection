...
def cache_v_stack_variable(v_stack_variable):
    ...
    tf.compat.v1.add_to_collection(UPDATE_V_STACK, update_v_stack_cache)
    reset_cache = cache.assign(tf.zeros_like(cache))
    tf.compat.v1.add_to_collection(fast_nn.RESET_CACHE_COLLECTION, reset_cache)
    return cache
...
def model_spec(row_input,
    ...
    with arg_scope(
        ...
        with arg_scope(
            ...
            cache_v_stack = tf.group(*tf.compat.v1.get_collection(UPDATE_V_STACK))
