...
def log_prob_from_logits(x):
    ...
    return x - m - tf.math.log(tf.reduce_sum(tf.exp(x-m), axis, keep_dims=True))
...
def discretized_mix_logistic_loss(x,l,sum_all=True):
    ...
    log_probs = tf.select(x < -0.999, log_cdf_plus, tf.select(x > 0.999, log_one_minus_cdf_min, tf.select(cdf_delta > 1e-5, tf.math.log(tf.maximum(cdf_delta, 1e-12)), log_pdf_mid - np.log(127.5))))
...
def sample_from_discretized_mix_logistic(l,nr_mix,seed=None):
    ...
    sel = tf.one_hot(tf.argmax(logit_probs - tf.math.log(-tf.math.log(tf.random.uniform(logit_probs.get_shape(), minval=1e-5, maxval=1. - 1e-5, seed=seed))), 3), depth=nr_mix, dtype=tf.float32)
    ...
    u = tf.random.uniform(means.get_shape(), minval=1e-5, maxval=1. - 1e-5, seed=(seed + 1 if seed is not None else None))
...
def get_var_maybe_avg(var_name, ema, **kwargs):
    ...
    v = tf.compat.v1.get_variable(var_name, **kwargs)
    if ema is not None:
        v = ema.average(v)
    return v
...
def dense(x, num_units, nonlinearity=None, init_scale=1., counters={}, init=False, ema=None, **kwargs):
    ...
    with tf.compat.v1.variable_scope(name):
        if init:
            ...
            V = tf.compat.v1.get_variable('V', [int(x.get_shape()[1]),num_units], tf.float32, tf.random_normal_initializer(0, 0.05), trainable=True)
            V_norm = tf.nn.l2_normalize(V.initialized_value(), [0])
            x_init = tf.matmul(x, V_norm)
            m_init, v_init = tf.nn.moments(x_init, [0])
            scale_init = init_scale/tf.sqrt(v_init + 1e-10)
            g = tf.compat.v1.get_variable('g', dtype=tf.float32, initializer=scale_init, trainable=True)
            b = tf.compat.v1.get_variable('b', dtype=tf.float32, initializer=-m_init*scale_init, trainable=True)
            ...
            V = tf.compat.v1.get_variable('V', [int(x.get_shape()[1]),num_units], tf.float32)
            g = tf.compat.v1.get_variable('g', [num_units], tf.float32)
            b = tf.compat.v1.get_variable('b', [num_units], tf.float32)
            if ema is not None:
...
def gated_resnet(x, a=None, h=None, nonlinearity=concat_elu, conv=conv2d, init=False, counters={}, ema=None, dropout_p=0., **kwargs):
    ...
    if h is not None:
        with tf.compat.v1.variable_scope(get_name('conditional_weights', counters)):
