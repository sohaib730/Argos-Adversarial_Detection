...
def _create_cache(batch, cache_height, cache_width, channels):
    ...
    tf.compat.v1.add_to_collection(RESET_CACHE_COLLECTION, reset_cache)
    return cache


def reset_cache_op():
    '''Returns an op to reset all created caches. Used between different generation calls.'''
    return tf.group(*tf.compat.v1.get_collection(RESET_CACHE_COLLECTION))


def _get_conv_variables(filter_size, input_channels, scope_name, counters):
    '''Creates and returns variables used for convolution.'''
    filter_height, filter_width, filter_channels = filter_size
    with tf.compat.v1.variable_scope(nn.get_name(scope_name, counters)):
        V = tf.compat.v1.get_variable(
            'V',
            [filter_height, filter_width, input_channels, filter_channels],
            dtype=tf.float32)
        g = tf.compat.v1.get_variable('g', [filter_channels], dtype=tf.float32)
        b = tf.compat.v1.get_variable('b', [filter_channels], dtype=tf.float32)
    return V, g, b
...
def _mod_equal_0(row_or_col, every):
    ...
    return tf.equal(tf.math.mod(row_or_col, every), 0)
...
def _conditional_info(h, batch, filter_channels, counters):
    ...
    with tf.compat.v1.variable_scope(nn.get_name('conditional_weights', counters)):
        hw = tf.compat.v1.get_variable(
            'hw',
