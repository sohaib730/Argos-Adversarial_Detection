...
def unpickle(file):
    ...
    if (sys.version_info >= (3, 0)):
        ...
        d = cPickle.load(fo)
    fo.close()
    return {'x': d['data'], 'y': np.array(d['labels']).astype(np.uint8)}


def load(data_dir, subset='train'):
    if subset=='train':
        train_data = [unpickle(os.path.join(data_dir,'batch_' + str(i))) for i in range(1,6)]
        trainx = np.concatenate([d['x'] for d in train_data],axis=0)
        trainy = np.concatenate([d['y'] for d in train_data],axis=0)
        return trainx, trainy
    elif subset=='test':
        test_data = unpickle(os.path.join(data_dir,'validation'))
        testx = test_data['x']
        testy = test_data['y']
        return testx, testy
    else:
