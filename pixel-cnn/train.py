"""
Trains a Pixel-CNN++ generative model on cifar or imagenet or gtsrb.
Uses multiple GPUs to speed up, indicated by the flag --nr_gpu

Example usage:
python train.py --nr_gpu 4
...
parser.add_argument('-o', '--save_dir', type=str, default='/model_cifar', help='Location for parameter checkpoints and samples')
...
parser.add_argument('-x', '--max_epochs', type=int, default=60, help='How many epochs to run in total?')
...
args.save_dir = f"Model_{args.data_set}"
...
if args.data_set == 'cifar':
    ...
    DataLoader = gtsrb_data.DataLoader
else:
...
with tf.Session() as sess:
    for epoch in range(args.max_epochs):
        ...
        print("Iteration %d, time = %ds, train bits_per_dim = %.4f, test bits_per_dim = %.4f" % (epoch, time.time()-begin, train_loss_gen, test_loss_gen),flush=True)
        sys.stdout.flush()
