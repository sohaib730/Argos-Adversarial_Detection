...
class DataLoader(object):
    ...
    def __init__(self, path):
        train_filenames = ['batch_{}'.format(ii+1) for ii in range(5)]
        eval_filename = 'test'


        train_images = np.zeros((35000, 32, 32, 3), dtype='uint8')
        train_labels = np.zeros(35000, dtype='int32')
        for ii, fname in enumerate(train_filenames):
            cur_images,cur_labels = self._load_datafile(os.path.join(path, fname))

            train_images[ii * 7000 : (ii+1) * 7000, ...] = cur_images
            train_labels[ii * 7000 : (ii+1) * 7000, ...] = cur_labels
